...
async def generate_recommendations(intention_text):
    ...
    try:
        ...
        # recommendations = {
        #     "extraction_goal": intention_text,
        #     "recommended_fields": [
        #         {
        #             "field_name": "material_name",
        #             "description": "Name or chemical formula of the synthesized material",
        #             "data_type": "string",
        #             "unit": None,
        #             "example": "TiO2 nanoparticles",
        #             "validation_rules": "Non-empty string"
        #         },
        #         {
        #             "field_name": "synthesis_temperature",
        #             "description": "Temperature at which synthesis was performed",
        #             "data_type": "number",
        #             "unit": "Celsius",
        #             "example": "450.0",
        #             "validation_rules": "Range: -273 to 3000"
        #         },
        #         {
        #             "field_name": "synthesis_method",
        #             "description": "Method or process used for material synthesis",
        #             "data_type": "string",
        #             "unit": None,
        ...
        #         },
        #         {
        #             "field_name": "reaction_time",
        #             "description": "Duration of the synthesis reaction",
        #             "data_type": "number",
        #             "unit": "hours",
        #             "example": "2.5",
        ...
        #             "description": "Size of resulting particles or crystallites",
        #             "data_type": "number",
        #             "unit": "nanometers",
        #             "example": "15.2",
        #             "validation_rules": "Positive number"
        #         }
        #     ],
        #     "additional_notes": "Consider extracting precursor materials and post-processing steps if mentioned."
        # }
...
async def generate_pydantic_model():
    ...
    try:
        ...
        # schema = app_state.orchestrator.model_agent.generate_model(app_state.current_recommendations)
        # app_state.current_schema = schema
        
        # Mock schema for demo
#         model_code = '''from pydantic import BaseModel, Field, field_validator
# from typing import Optional

# class MaterialSynthesis(BaseModel):
#     """Extracted material synthesis information"""
...
#         default=None,
#         description="Temperature at which synthesis was performed (Celsius)"
#     )
...
#         default=None,
#         description="Duration of the synthesis reaction (hours)"
#     )
    
#     particle_size: Optional[float] = Field(
#         default=None,
#         description="Size of resulting particles or crystallites (nanometers)"
#     )
    
#     @field_validator("synthesis_temperature")
#     @classmethod
#     def validate_temperature(cls, v):
#         if v is not None and (v < -273 or v > 3000):
#             raise ValueError("Temperature out of valid range")
#         return v
    
#     @field_validator("reaction_time", "particle_size")
#     @classmethod
...
#         extraction_prompt = """Extract material synthesis information from scientific papers.

# For each document, identify:
# - Material names (chemical formulas or common names)
# - Synthesis temperatures (convert to Celsius if needed)
# - Synthesis methods (sol-gel, CVD, hydrothermal, etc.)
# - Reaction times (convert to hours if needed)
# - Particle sizes (convert to nanometers if needed)

# Be precise with numerical values and units. If information is not explicitly stated, use null."""
...
def index_data_for_rag(progress=gr.Progress()):
    """Index extracted data for RAG queries"""
    if not app_state.extracted_data:
        return "❌ No data to index. Complete extraction first.", gr.update(interactive=False)
    
    try:
        progress(0, desc="Indexing data...")
        
        # Call RAG agent indexing
        # app_state.rag_agent.index_data(
        #     data=app_state.extracted_data,
        #     collection_name=f"extraction_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        # )
        ...
        return (
            "✅ **Data Indexed Successfully!**\n\nYou can now ask questions about your extracted data.",
            gr.update(interactive=True)
        )
...
def query_data(query_text, num_results):
    """Query indexed data using RAG agent"""
    if not app_state.indexed:
        return (
            "❌ Please index data first by clicking 'Index Data for Queries'.",
            None,
            None,
            None
        )
    ...
    try:
        # Call RAG agent
        # result = app_state.rag_agent.query(query_text, n_results=num_results)
        
        # Mock result for demo
        result = {
            "answer": "Based on the extracted data, three materials were synthesized at temperatures above 400°C:\n\n1. **TiO2 Nanoparticles** at 450°C using sol-gel process\n2. **Silicon Nanowires** at 850°C using Chemical Vapor Deposition\n\nThe silicon nanowires were synthesized at the highest temperature (850°C), which is suitable for CVD processes requiring high thermal energy.",
            "columns_used": ["synthesis_temperature", "material_name", "synthesis_method"],
            "confidence": 0.95,
            "additional_insights": "Higher synthesis temperatures generally correlate with larger particle sizes in this dataset. The CVD method requires significantly higher temperatures than sol-gel or hydrothermal methods.",
            "relevant_records": [
                {
                    "material_name": "Silicon Nanowires",
                    "synthesis_temperature": 850.0,
                    "synthesis_method": "Chemical Vapor Deposition",
                    "_similarity_score": 0.92
                },
                {
                    "material_name": "TiO2 Nanoparticles",
                    "synthesis_temperature": 450.0,
...
**📊 Columns Used:** {', '.join([f'`{c}`' for c in result['columns_used']])}
        """
...
{result['additional_insights']}
        ...
        # Create dataframe of relevant records
        records_df = pd.DataFrame(result['relevant_records'])
        ...
        return (
            ...
            records_df,
            fig,
            f"Query processed successfully using {len(result['relevant_records'])} relevant records."
        )
    ...
    with gr.Blocks(theme=gr.themes.Soft(), css=custom_css, title="AI Document Extraction") as demo:
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.Tab("🔍 Query Data", id=1):
                ...
                Use natural language to query and analyze your extracted information.
                """)
